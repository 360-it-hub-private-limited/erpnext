...
def create_journal_entry_bts(
    ...
    # Check if currency is None
    if not bank_transaction.currency:
        frappe.throw(_("Currency is missing for the bank transaction: {0}").format(bank_transaction_name))
    ...
    if is_multi_currency:
        ...
        if exc_rate is None:
            frappe.throw(_("Exchange rate not found for {0} to {1} on {2}").format(
                bank_transaction.currency, company_default_currency, posting_date
            ))
    ...
    if second_account_currency != company_default_currency:
        ...
        if exc_rate is None:
            frappe.throw(_("Exchange rate not found for {0} to {1} on {2}").format(
                second_account_currency, company_default_currency, posting_date
            ))
    ...
    if company_account_currency != company_default_currency:
        ...
        if exc_rate is None:
            frappe.throw(_("Exchange rate not found for {0} to {1} on {2}").format(
                company_account_currency, company_default_currency, posting_date
            ))
